...
function setup() {
    ...
    // --- comentar en OPC
    sliders_activate();
    OPC.slider("SEED",         442, 1, 1000, 1);
    OPC.slider("TENS_DISPLAY", 0, 0, 2, 1);  // 0.normal, 1.texture, 2.debug, no 3.perlin 
    OPC.slider("TENS_MOVE",    1, 0, 2, 1);  // 0.still, 1.curtain, 2. test
    OPC.slider("ROWS",         22, 2, 30, 1);
    OPC.slider("DOTS_ROW",     38, 3, 50, 1);
    OPC.slider("COLOR_MODE",   3, 0, 3, 1);  
    OPC.slider("DOT_SPACING_RANGE",   0.2, 0, 0.5, 0.05);  
    OPC.slider("DOT_HEIGHT_RANGE",   0.2, 0, 1, 0.1);  
    OPC.slider("DOT_BORDERS",   0.4, 0, 0.6, 0.1);  


    OPC.slider("MOVE_MODE", 2, 0, 3, 1);
    OPC.slider("MOVE_MODE_SPEED", 273, 100, 1000, 1);
    OPC.slider("ROW_DANCE", 0, 0, 1, 1);

    OPC.slider("_deprecated_next_", 999, 999, 1000, 1);
    ...
    OPC.slider("PERIOD_X",      150, 50, 200,  1);
    OPC.slider("PERIOD_Y",      150, 50, 200,  1);
...
function draw() {
    ...
    if (TENS_DISPLAY == 2) {
        ...
        if (TENS_MOVE == 0) {         // gradation only if not moving
