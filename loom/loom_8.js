...
function setup() {
    ...
    // --- comentar en OPC
    sliders_activate();
    sliders_add(1, 1000, 408, 1, "SEED");
    sliders_add(0, 1, 1, 1, "MODE");   // 0.loom, 1.neurons
    sliders_add(0, 3, 0, 1, "NOISE_MODE"); 
    sliders_add(6, 20, 14, 1, "ROWS");
    sliders_add(3, 250, 16, 1, "DOTS_ROW");
...
function draw() {
    ...
    step++;
    if (step % (20 + MODE_FORCED * 100) == 0) {
        MODE_FORCED = 1 - MODE_FORCED;
    }
}
...
class Tenso {
    ...
    constructor() {
        ...
        this.row_0_stroke = 3;
        this.rows = ROWS;
        this.off_x = (window_width - this.size)/2;
        this.off_y = (window_height - this.size)/2;
    ...
    _draw_triangle(dot_a, dot_b, dot_c, col, k = 0.05) {
        switch (NOISE_MODE) {
            case 1:   // subtriangles
                perlin_triangle_2(this.off_x + dot_a.x, this.off_y + dot_a.y, this.off_x + dot_b.x, this.off_y + dot_b.y, this.off_x + dot_c.x, this.off_y + dot_c.y, col, k, 60, 12);
            break;
            case 2:     // WIP
                perlin_triangle_2(this.off_x + dot_a.x, this.off_y + dot_a.y, this.off_x + dot_b.x, this.off_y + dot_b.y, this.off_x + dot_c.x, this.off_y + dot_c.y, col, k, 60, 12);
            break;
    ...
    display_triangles_perlin(pixels) {    // works for modes 3
        ...
        for (let row = 1; row < a.length ; row++) {                     // from row 1 to last
            for (let i = 0; i < a[row].length; i++ ) {                  // every point in row
                ...
                for (let n = 1; n < a[row][i].neighbours.length; n++) {  // dots for base
                    ...
                    //triangle(this.off_x + dot_a.x, this.off_y + dot_a.y, this.off_x + dot_b.x, this.off_y + dot_b.y, this.off_x + dot_c.x, this.off_y + dot_c.y);
                    //this._draw_triangle(dot_a, dot_b, dot_c, c_palette[triangle_color%c_palette.length]);
                    this.perlin_triangle_4(pixels, this.size, dot_a.x, dot_a.y, dot_b.x, dot_b.y, dot_c.x, dot_c.y, rc, gc, bc);
                    //triangle_color++;
                }
    ...
    perlin_triangle_4(img_px, img_w, x1, y1, x2, y2, x3, y3, rcol, gcol, bcol, k=0.05, pixel_size=1, color_range=30) {
        ...
        for(let i = X1; i <= X2; i+=pixel_size){
            for(let j = Y1; j <= Y2; j+=pixel_size){
                ...
                if (cross_lines == 1) {
                    ...
                    b = map (b, 0, 1, bcol-color_range, bcol+color_range);
                    r = map (r, 0, 1, rcol-color_range, rcol+color_range);
                    g = map (g, 0, 1, gcol-color_range, gcol+color_range);
                                
                    //	write_color(img, x, y, r, g, b, alpha);
                    let index = floor(((this.off_x + i) + (this.off_y + j) * window_width) * 4 * pixel_density);
                    img_px[index] = r;
                    img_px[index + 1] = g;
                    img_px[index + 2] = b;
                    img_px[index + 3] = 255;
